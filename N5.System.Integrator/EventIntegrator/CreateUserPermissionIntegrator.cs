...
[KafkaTopicAttribute(UserEvent.GeneratePermission)]
public class CreateUserPermissionIntegrator(IMediator _mediator,
                                            IKafkaProducerService _kafkaProducerService) : IKafkaEvent
{
    ...
    {
        var entityMessage = meesage.Value.ToDeserializeJSON<CreateUserPermissionDto>();
        var entityMap = MapperConfig.Mapper.Map<CreateUserPermissionCommand>(entityMessage);
